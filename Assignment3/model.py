...
class ParserModel(Model):
    ...
    def add_embeddings(self):
        ...
        ** Embedding matrices should be variables, not constants! **
        ...
        word_lookup = self.word_embeddings
        word_embeddings = tf.nn.embedding_lookup(word_lookup, self.word_id_placeholder)
        word_embeddings = tf.reshape(word_embeddings,[-1,self.config.n_word_features*self.config.embed_size])

        tag_lookup = xavier_initializer([self.config.n_tag_ids,self.config.embed_size])
        tag_embeddings = tf.nn.embedding_lookup(tag_lookup, self.tag_id_placeholder)
        tag_embeddings = tf.reshape(tag_embeddings,[-1,self.config.n_tag_features*self.config.embed_size])
        deprel_lookup = xavier_initializer([self.config.n_deprel_ids,self.config.embed_size])
        deprel_embeddings = tf.nn.embedding_lookup(deprel_lookup, self.deprel_id_placeholder)
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### BEGIN YOUR CODE
        lossTensor = tf.nn.softmax_cross_entropy_with_logits(labels = self.class_placeholder, logits = pred)
        loss = tf.reduce_mean(lossTensor)
        ### END YOUR CODE
        return loss
