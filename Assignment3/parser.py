...
class PartialParse(object):
	...
	def parse_step(self, transition_id, deprel=None):
		...
		'''
		### BEGIN STUDENT CODE
		if transition_id > 2 or transition_id < 0:
	...
	def get_n_leftmost_deps(self, sentence_idx, n=None):
		...
		for i in range(len(self.arcs)):
			if (self.arcs[i][0] == sentence_idx) and count < n:
	...
	def get_n_rightmost_deps(self, sentence_idx, n=None):
		...
		for i in reversed(range(len(self.arcs))):
			if (self.arcs[i][0] == sentence_idx) and count < n:
				deps.append(self.arcs[i][1])
				count += 1
		### END STUDENT CODE
	...
	def get_oracle(self, graph):
		...
		*ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
		...
		### BEGIN STUDENT CODE
		if self.stack == [0]:
			transition_id, deprel = self.shift_id, 'deprel'
		else:
			transition_id,deprel = self.shift_id, 'deprel'
			for i in [-2,-1]:
				deps = graph.nodes[self.stack[i]]['deps']
				other = (i % -2) - 1
				otherdeps = graph.nodes[self.stack[other]]['deps']
				otherarcs = [arc for arc in self.arcs if arc[0]==self.stack[other]]
				if len(deps) > 0 and len(otherarcs) == len(otherdeps):
					for key in deps:
						if deps[key][0] == self.stack[other]:
							transition_id,deprel = 1 - abs(i % -2), key
		### END STUDENT CODE
		if transition_id == 0:
			transition_id = self.left_arc_id
		elif transition_id == 1:
...
def minibatch_parse(sentences, model, batch_size):
	...
	while len(unfinished_parses) > 0:
		...
		td_pairs = model.predict(batch)
		popped = 0
		for i in range(len(batch)):
			td = td_pairs[i]
			try:
				batch[i].parse_step(td[0],td[1])
				if batch[i].complete:
					unfinished_parses.pop(i-popped)
					popped+=1
			except:
				unfinished_parses.pop(i-popped)
				popped+=1
